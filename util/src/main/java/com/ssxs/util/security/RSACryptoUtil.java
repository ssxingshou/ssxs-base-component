...
public class RSACryptoUtil {
     ...
     * 保存生成的密钥对的文件名称。
     */
    private static final String WXPAY_RSA_PAIR_FILENAME = "/public.pem";
    /**
     * 密钥大小
     */
    ...
    private static File wxpayRsaPairFile = null;

    //private static String urlPath = "/data/rsaHome";
    private static String urlPath = "D:/rsaHome";
    ...
    static {
        ...
        rsaPairFile = new File(getRSAPairFilePath(urlPath));
        wxpayRsaPairFile = new File(getWxpayRSAPairFilePath(urlPath));
    }
    ...
    private static synchronized KeyPair generateKeyPair() {
        try {
            ...
            keyPairGen.initialize(KEY_SIZE, new SecureRandom(formatted.getBytes()));
     ...
     * 返回生成/读取的密钥对文件的路径。
     */
    private static String getRSAPairFilePath(String urlPath) {
        return (new File(urlPath) + RSA_PAIR_FILENAME);
    }

    private static String getWxpayRSAPairFilePath(String urlPath) {
        return (new File(urlPath) + WXPAY_RSA_PAIR_FILENAME);
    }

    /**
     * 若需要创建新的密钥对文件，则返回true，否则false。
     */
     ...
     * 若需要创建新的密钥对文件，则返回true，否则false。
     */
    private static boolean isCreateWxpayKeyPairFile() {
        // 是否创建新的密钥对文件
        boolean createNewKeyPair = false;
        if (!wxpayRsaPairFile.exists() || wxpayRsaPairFile.isDirectory()) {
            createNewKeyPair = true;
        }
        return createNewKeyPair;
    }
    ...
    /**
     * 使用给定的公钥加密给定的字符串。
     * 若 publicKey 为 null，或者 plaintext 为 null 则返回 null。
     *
     * @param publicKey 给定的公钥。
     * @param plaintext 字符串。
     * @return 给定字符串的密文。
    ...
    /**
     * 使用给定的私钥解密给定的字符串。
     * 若私钥为null，或者 encrypttext 为 null或空字符串则返回null。
     * 私钥不匹配时，返回 null。
     *
     * @param privateKey  给定的私钥。
    ...
    /**
     * 使用默认的私钥解密给定的字符串。
     * 若encrypttext 为 null或空字符串则返回 null。
     * 私钥不匹配时，返回 null。
     *
     * @param encrypttext 密文。
     * @return 原文字符串。
